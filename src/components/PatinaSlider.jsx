...
export function PatinaSlider({ currentYear, onYearChange }) {
  ...
  return (
    <div className="absolute top-20 left-4 z-20 pointer-events-auto hidden md:block max-w-[220px] p-3 glass-panel rounded-2xl shadow-warm border border-[#5C3A21]/15">
      <div className="flex items-center gap-1.5 mb-1">
        <Clock className="w-3.5 h-3.5 text-[#8C5A3C]" />
        <span className="text-[10px] uppercase tracking-widest text-[#24140E] font-bold">
          Pamana Aging Simulator
        </span>
      </div>
      ...
      {/* Year Pill Selector */}
      <div className="flex items-center justify-between gap-1 p-1 bg-[#F2ECE4]/70 rounded-xl">
        {years.map((y) => (
          <button
            key={y}
            onClick={() => {
      ...
      <div className="mt-2 flex items-center gap-1 text-[9px] text-[#8C5A3C] font-semibold">
        <ShieldCheck className="w-3 h-3 text-[#C4975D]" />
        <span>Lifetime natural craft warranty</span>
      </div>
    </div>
  );
}
